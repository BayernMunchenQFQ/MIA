...
if __name__ == '__main__':
    ...
    for thenumberofexperiment in range(2):
        ...
        for i in range(1, N + 1):
            ...
            LRN = random.randint(1, 10)
            shadow_C = random.randint(1, 10)
            # print(f'MAX_EPOCH:      {MAX_EPOCH}')
            print(f"LRN:             {LRN}")
            print(f"shadoa_C:         {shadow_C}")
            """
            # shadow_model = svm.SVC(C=shadow_C, kernel="rbf", probability=True, tol=LRN * 1e-4)
        ...
        attack_model_scores1 = [x-min(attack_model_scores) for x in attack_model_scores]
        score_sum1 = 0
        for item in attack_model_scores1:
            score_sum1 += value(item*1000)
        attack_model_weights1 = [value(score*1000) / score_sum1 for score in attack_model_scores1]

        attack_model_scores2 = [(score - np.mean(attack_model_scores)) / np.std(attack_model_scores) for score in attack_model_scores]
        score_sum2 = 0
        for item in attack_model_scores2:
            score_sum2 += value(item)
        attack_model_weights2 = [value(score) / score_sum2 for score in attack_model_scores2]

        attack_model_scores3 = [(score - min(attack_model_scores)) / (max(attack_model_scores) - min(attack_model_scores)) for score in attack_model_scores]
        score_sum3 = 0
        for item in attack_model_scores3:
            score_sum3 += value(item)
        attack_model_weights3 = [value(score) / score_sum3 for score in attack_model_scores3]
        ####################################################################################################################

        ####################################################################################################################
        print("membership inference attack...")
        LM_test_x1 = [0 for i in range(len(LM_test_x))]
        # 将不同的攻击模型对同一条数据的分类分数进行加权平均计算得到最后的分类分数，已进行最终分类
        for k in range(len(LM_test_x)):
            score = 0
            for h in range(N):
                score += attack_model_weights1[h] * LM_test_x[k][h]
            LM_test_x1[k] = score
        print(LM_test_x1)
        test_result = [1 if p > 0.5 else 0 for p in LM_test_x1]
        # 计算并输出最终的分类结果
        test_right_num = 0
        TP = 0
        FP = 0
        ...
        print(
            "-----------------------------------------------------------------------------------------------------------------")
        # 将不同的攻击模型对同一条数据的分类分数进行加权平均计算得到最后的分类分数，已进行最终分类
        LM_test_x2 = [0 for i in range(len(LM_test_x))]
        for k in range(len(LM_test_x)):
            score = 0
            for h in range(N):
                score += attack_model_weights2[h] * LM_test_x[k][h]
            LM_test_x2[k] = score
        print(LM_test_x2)
        test_result = [1 if p > 0.5 else 0 for p in LM_test_x2]
        # 计算并输出最终的分类结果
        test_right_num = 0
        TP = 0
        FP = 0
        ...
        print(
            "-----------------------------------------------------------------------------------------------------------------")
        # 将不同的攻击模型对同一条数据的分类分数进行加权平均计算得到最后的分类分数，已进行最终分类
        LM_test_x3 = [0 for i in range(len(LM_test_x))]
        for k in range(len(LM_test_x)):
            score = 0
            for h in range(N):
                score += attack_model_weights3[h] * LM_test_x[k][h]
            LM_test_x3[k] = score
        print(LM_test_x3)
        test_result = [1 if p > 0.5 else 0 for p in LM_test_x3]
        # 计算并输出最终的分类结果
        test_right_num = 0
        TP = 0
        FP = 0
        ...
        for i in range(N):
            ...
            print(f"weight1 {attack_model_weights1[i]}")
            print(f"weight2 {attack_model_weights2[i]}")
            print(f"weight3 {attack_model_weights3[i]}")
